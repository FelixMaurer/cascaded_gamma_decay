...
def get_orthogonal_vectors(v):
    ...
    # Cross product for the third orthogonal unit vector
    u2 = np.cross(v, u1)
    return u1, u2
...
def get_orbit_points(normal, radius, phase=0.0, points=60):
    ...
    # Generate points starting at the phase offset
    theta = np.linspace(phase, 2 * np.pi + phase, points)
    orbit = np.array([radius * np.cos(t) * u1 + radius * np.sin(t) * u2 for t in theta])
    
    # Calculate the exact tangent direction at the starting point (theta = phase)
    # Derivative of: cos(theta)*u1 + sin(theta)*u2 -> -sin(theta)*u1 + cos(theta)*u2
...
def plot_3d_state(state_name, vectors, show_orbits, show_vectors, show_charge, show_core, beta, radii, phases, colors, names):
    ...
    for i, v in enumerate(vectors):
        ...
        # 2. Orbits (perpendicular to exact j vectors, staggered by phase)
        if show_orbits:
            ...
            # Moving direction arrow (tangent cone)
    ...
    # Extended axis ranges [-6, 6] to ensure nothing is cut off
    fig.update_layout(
...
# Phase arrays offset the nucleons so they never overlap identically
...
def plot_transition_animation():
    ...
    fig = go.Figure(
        ...
        layout=go.Layout(
            scene=dict(
                xaxis=dict(range=[-6, 6], visible=False), # Expanded bounds
                yaxis=dict(range=[-6, 6], visible=False),
                zaxis=dict(range=[-6, 6], visible=False),
                aspectmode='cube'
            ),
